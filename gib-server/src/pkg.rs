...
async fn create_pkg(mut playload: Multipart) -> HttpResponse {
    while let Ok(Some(mut field)) = playload.try_next().await {
        ...
        let tar_gz = match File::open(filepath.as_str()) {
            Err(e) => {
                return HttpResponse::InternalServerError()
                    ...
                    .body(format!(
                        r#"{{"status": 1, "error": "Error processing archive: {}"}}"#,
                        e
                    ));
            }
            Ok(e) => e,
        ...
        for x in archive.entries().unwrap().filter(|x| {
            x.as_ref()
                .unwrap()
                .path()
                .unwrap()
                .ends_with("package.toml")
        }) {
        ...
        if !re.is_match(data.name.as_str()) {
            return HttpResponse::BadRequest()
                .header("Content-Type", "application/json")
                .body(format!(
                    r#"{{"status": 2, "error": "Invalid name in package.toml, provided: {}"}}"#,
                    data.name.clone()
                ));
        }
        ...
        let package = match create_pacakge(
            ...
            Err(e) => {
                return HttpResponse::InternalServerError()
                    .header("Content-Type", "application/json")
                    .body(format!(
                        r#"{{"status": 4, error: "Error creating package in database: {}" "#,
                        e
                    ))
            }
        };
        ...
        match upload_package_archive(
            ...
            Err(e) => {
                return HttpResponse::InternalServerError()
                    .header("Content-Type", "application/json")
                    .body(format!(
                        r#"{{"status": 4, error: "Error uploading archive to database: {}" "#,
                        e
                    ))
            }
        };
...
fn get_package(request: HttpRequest) -> HttpResponse {
    ...
    if !packages.is_empty() {
        return HttpResponse::Ok()
            ...
            .body(format!(
                r#"{{"status": "0", "package": {}}}"#,
                serde_json::to_string(packages.get(0).unwrap()).unwrap()
            ));
    }
    ...
    return HttpResponse::BadRequest()
        .header("Content-Type", "application/json")
        .body(format!(
            r#"{{"status": "7", "error": "no package found called: {}"}}"#,
            package
        ));
}
...
async fn get_package_latest(request: HttpRequest) -> HttpResponse {
    ...
    if archives.is_empty() {
        return HttpResponse::BadRequest()
            .header("Content-Type", "application/json")
            .body(format!(
                r#"{{"status": "7", "error": "no archive forund for: {}@{}"}}"#,
                package.package_name, package.current_version
            ));
    }
